...
class MultipleShooting(ConstrainedSolver):
    def __init__(
        ...
        self._w_dims = self._n_steps * (self._u_dims + self._x_dims) + self._x_dims
        ...
        self._w_u_max = jnp.full(shape=(self._u_dims * self._n_steps,), fill_value=self._u_max, dtype=jnp.float32)
        self._w_u_min = jnp.full(shape=(self._u_dims * self._n_steps,), fill_value=self._u_min, dtype=jnp.float32)

        # constraints
        # equality constraints on the last state x(N)
        self._eq_mult_dims = self._x_dims * (self._n_steps + 1)
        ...
        self._ineq_mult_dims = 2 * self._n_steps * self._u_dims
    ...
    def solve(self) -> Tuple[jnp.ndarray, Tuple]:
        ...
        while k < self._max_iter:
            ...
            c_eq_k, grad_c_eq_k = self._eval_eq_constraints_with_grads(
                w_k, k, time_grid_shooting, dfds_fn, dfdu_fn)

            c_ineq_k = self._eval_ineq_constraints(x=w_k, time_grid_shooting=time_grid_shooting)
    ...
    def _build_w_penalty_ref(self):
        p = []
        r = []
        u_ref = jnp.zeros((self._u_dims,), dtype=jnp.float32)
    ...
    def _build_grad_ineq_matrix(self):
        grad_c_ineq_k = jnp.zeros((self._ineq_mult_dims, self._w_dims), jnp.float32)

        step = self._x_dims + self._u_dims
        for i, i_x in enumerate(range(self._x_dims, self._w_dims, step)):
            i_y = i * 2 * self._u_dims
            grad_c_ineq_k = grad_c_ineq_k.at[i_y, i_x].set(1.)
            grad_c_ineq_k = grad_c_ineq_k.at[i_y + 1, i_x].set(-1.)

        return grad_c_ineq_k

    def _split_by_x_u(self, w_k):
        n = self._n_steps
        n_x = self._x_dims
        n_u = self._u_dims
        ...
        xs = jnp.reshape(w_k_x, (self._n_steps + 1, self._x_dims))

        # extract u
        u_vals = list(range(n_x, n_x + n_u))
        u_ind = [u_val + i * n_x + i * n_u for i in range(n) for u_val in u_vals]
        w_k_u = jnp.take(w_k, u_ind)
        us = jnp.reshape(w_k_u, (self._n_steps, self._u_dims))

        return xs, us

    def _ms_loss_fn(self, w_k, k, with_grads=False):
        res = w_k - self._w_ref
    ...
    def _eval_eq_constraints_with_grads(self, w_k, k, time_grid_shooting, dfds_fn, dfdu_fn):
        ...
        diag_component = jnp.diag(jnp.ones((self._x_dims,), jnp.float32))
        grad_c_eq_k = jnp.zeros((self._eq_mult_dims, self._w_dims), jnp.float32)
        grad_c_eq_k = grad_c_eq_k.at[:self._x_dims, :self._x_dims].set(diag_component)
        ...
        for i, (t_start, t_end, x_i, x_j, u_i) in enumerate(zip(
            ...
            # fill G_x, G_u
            ind_x_axis = i * (self._x_dims + self._u_dims)
            dxdw_next = dxdw_next.at[:, ind_x_axis:ind_x_axis + self._x_dims].set(G_x)
            dxdw_next = dxdw_next.at[:, ind_x_axis + self._x_dims].set(G_u)

            ind_y_axis = (i + 1) * self._x_dims
            grad_c_eq_k = grad_c_eq_k.at[ind_y_axis:ind_y_axis + self._x_dims, :].set(dxdw_next)

            # fill diag(-1)
            diag_ind = ind_x_axis + self._x_dims + self._u_dims
            grad_c_eq_k = grad_c_eq_k.at[ind_y_axis:ind_y_axis + self._x_dims, diag_ind:diag_ind + self._x_dims].set(
                -diag_component)
    ...
    def _eval_ineq_constraints(self, x, **kwargs):
        ...
        c_ineq_k = jnp.empty((2 * self._u_dims * self._n_steps,), jnp.float32)
